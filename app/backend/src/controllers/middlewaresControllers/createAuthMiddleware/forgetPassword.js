...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const ubinarys_app_email = settings['ubinarys_app_email'];
  const ubinarys_base_url = settings['ubinarys_base_url'];

  const url = checkAndCorrectURL(ubinarys_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | ubinarys',
    ubinarys_app_email,
